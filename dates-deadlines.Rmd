...
mkdeadlinetable <- function(url){
  jsonlite::fromJSON(readr::read_file(url)) %>%
  mutate(
  ...
  select(module, assessment, `Hand in date`, status) %>%
  set_names(., Hmisc::capitalize(names(.))) %>%
  pander::pander(split.tables=Inf, justify="llcc")
}
```
...
```{r, echo=F}
mkdeadlinetable('https://wam.psy.plymouth.ac.uk/admin/reporting/programme/msc-cp/deadlines/?format=json')
```

## MSc CP Deadlines

```{r, echo=F}
mkdeadlinetable('https://wam.psy.plymouth.ac.uk/admin/reporting/programme/msc-ap/deadlines/?format=json')
```
