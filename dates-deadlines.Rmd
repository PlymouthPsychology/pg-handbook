...
mkdeadlinetable <- function(url){
  ...
  #jsonlite::fromJSON(readr::read_file(url), flatten=T) %>%names
  ...
  set_names(., c("Module", "Name", "Staff responsible", "Module %", "Hand in date")) %>%
  pander::pander(split.tables=Inf, justify="lllcl")
}
```
